...
{{#TEX_MAP_TO_SPECULAR}}
	specular *= texture(u_Sampler{{TEX_INDEX}}, fs_TexCoords[{{UV_SET_INDEX}}].{{TEX_COORDS}});
{{/TEX_MAP_TO_SPECULAR}}

{{#TEX_MAP_TO_SHININESS}}
	shininess = texture(u_Sampler{{TEX_INDEX}}, fs_TexCoords[{{UV_SET_INDEX}}].{{TEX_COORDS}});
{{/TEX_MAP_TO_SHININESS}}

{{#TEX_MAP_TO_DIFFUSE}}

{{#TEX_BLENDOP_MULTIPLY}}
	texDiffuse *= texture(u_Sampler{{TEX_INDEX}}, fs_TexCoords[{{UV_SET_INDEX}}].{{TEX_COORDS}});
{{/TEX_BLENDOP_MULTIPLY}}

{{#TEX_BLENDOP_DECAL}}
	vec4 texColor{{TEX_INDEX}}= texture(u_Sampler{{TEX_INDEX}}, fs_TexCoords[{{UV_SET_INDEX}}].{{TEX_COORDS}});
	vec3 c{{TEX_INDEX}} = mix(texDiffuse.rgb, texColor{{TEX_INDEX}}.rgb, texColor{{TEX_INDEX}}.a);
